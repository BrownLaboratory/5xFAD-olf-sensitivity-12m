...
# summarize 5x data
dat5xSum <- dat5x %>%
  summarise('acc' = sum(correct)/length(correct)
            , 'crit' = acc >= .85)
#### Intro analysis ####
...
pointSize <-  2
plotAcc12m <- ggplot(data = filter(datCorrPlot
                     , conc == 1e0)
       , aes(x = xPos
             , y = acc
             , shape = paste(Genotype, sex)
  ...
  geom_errorbar(aes(ymin = ciLower
                ...
                , linetype = 1
                , width = .5) +
  geom_point(data = filter(datCorrPlot
                           , conc == 1e-1)
             , size = pointSize) +
  geom_line(data = filter(datCorrPlot
                          , conc == 1e-1)) +
  geom_errorbar(data = filter(datCorrPlot
                               , conc == 1e-1)
                 , aes(ymin = ciLower
                       , ymax = ciUpper)
                , linetype = 1
...
##### Plot False Alarms ####
plotFa12m <- ggplot(data = filter(sMinusSdat
                       , day == 1)
         , aes(x = xPos
               , y = fa$y
               , ymin = fa$ymin
  ...
  geom_point(size = pointSize) +
  geom_line() +
  geom_errorbar(data = sMinusSdat
                , linetype = 1
                , width = .5) +
  geom_point(data = filter(sMinusSdat
                           , day == 2)
             , size = pointSize) +
  geom_line(data = filter(sMinusSdat
                          , day == 2)) +
  ...
  scale_shape_manual(values = c(17, 16, 2, 1)) + # c(1, 2, 16, 17)) +
  scale_linetype_manual(values = c('dashed', 'dashed', 'solid', 'solid')) +
  scale_x_continuous(breaks = c(seq(3,23, by = 5), 30.5) # axisBreaks
                     , labels = c(format(concList[1]
                                         , nsmall = 0
                                         , scientific = FALSE)
                                  , format(concList[2]
...
faDat <- dat5x %>%
  mutate('day' = as.numeric(substr(StudyName, 4, 4))) %>%
  group_by(animal, block, day, geno, sex) %>%
  summarise('fa' = sum(!correct[TrialType == 'N']),
...
ssDat <- dat %>%
  filter(StudyName %in% paste0('SEN', 1:7)
         , SS == TRUE) %>%
  mutate('day' = as.numeric(substr(StudyName, 4, 4))) %>%
  group_by(animal, block, day, geno, sex) %>%
  summarise('N' = n()) #%>%
...
ggplot(data = filter(ssDat
  ...
  geom_point(size = pointSize) +
  geom_line() +
  geom_point(data = filter(ssDat
                           , day == 2)
             , size = pointSize) +
  geom_line(data = filter(ssDat
                          , day == 2)) +
  ...
  scale_x_continuous(breaks = axisBreaks
                     , labels = concList[1:6]) +
  labs(y = 'Short Sample (n)'
       , x = 'Odour Concentration (ppm)'
       , linetype = 'Genotype'
       , shape = 'Genotype'
       , colour = 'Sex') +
  theme(legend.position = 'none' #c(.9, .2)
        , legend.title = element_blank())
...
senDat <- dat5x %>%
  ...
  group_by(animal, geno, sex) %>%
  summarise('hits' = sum(correct[TrialType == 'P'])
...
ssDat %>%
  group_by(sex) %>%
  summarise(ssMean = mean(ShortSamples),
  ...
  scale_x_continuous(breaks = c(0:4, 5.5)
                     , labels = c(format(concList[1]
                                  ...
                                  , format(concList[6]
                                           , nsmall = 0
                                           , scientific = FALSE))) +
  labs(y = 'Short Samples (n)'
       , x = 'Odour Concentration (ppm)'
       , shape = '') +
  theme(legend.position = c(.8, .8)
        , legend.title = element_blank())

ggsave(here('plots', 'newNewPlots', 'shortSamples.png'),
       width = 8.5,
       height = 5.5)
